...
class LogsCollector:
    def collect(self, pod_name: str, namespace: str, tail_lines: int = 50) -> Dict[str, Any]:
        """
    ...
    def collect_for_unhealthy(self, problematic_pods: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        all_logs = []
        for pod in problematic_pods[:5]:  # Limit to first 5 problematic pods to keep payload reasonable
            all_logs.append(self.collect(pod["name"], pod["namespace"]))
        return all_logs
