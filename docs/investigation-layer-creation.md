...
6.  **Docker Credential Fix**: Resolved an `error getting credentials` issue by removing the problematic `"credsStore": "desktop.exe"` from the Docker configuration in the WSL environment.
7.  **Kubernetes Investigation Layer**: Built a set of tools to gather evidence from the cluster using `kubectl`. This layer acts as a "Junior DevOps Engineer" collecting data before the AI analyzes it.

---

## 🛠️ Troubleshooting Common Errors

### 1. `error getting credentials - err: exit status 1`
**Problem**: Docker in WSL tries to use a Windows-side credential helper (`desktop.exe`) that is unreachable or misconfigured.
**Fix**: Remove the `credsStore` line from `~/.docker/config.json`. This allows Docker to pull public images like Python and Node without hitting the broken helper.

### 2. `WSL ERROR: UtilAcceptVsock`
**Problem**: This often accompanies network timeouts or Docker service interruptions.
**Fix**: Usually resolved once the credential helper issue is fixed and the Docker service is restarted or the command is retried.
...
*   **How it works**: It runs multiple "Inspectors" in a specific order:
    1.  **Pods**: Finds what's broken.
    2.  **Logs**: Grabs the last 50 lines of logs for the broken pods.
    3.  **Events**: Looks for "Warning" events like `FailedScheduling`.
    4.  **Deployments**: Checks if the desired number of replicas matches what's actually running.
    5.  **Networking**: Checks if Services have actual Endpoints (pods) to talk to.

### 8. API Endpoint (`backend/main.py`)
```python
@app.post("/investigate")
async def investigate():
    return {"status": "success", "investigation": investigation_service.run_full_investigation()}
```
*   **Why?**: This exposes the investigation logic to the Frontend. One click from the user triggers a full cluster scan.
